...
/**
 * Copy from o.a.s.s.c.expressions.EquivalentExpressions
 */
class EquivalentBatchExpressions {
  /**
   * Wrapper around an Expression that provides semantic equality.
   */
  case class Expr(e: BatchExpression) {
  ...
  private val equivalenceMap = mutable.HashMap.empty[Expr, mutable.MutableList[BatchExpression]]
   ...
   */
  def addExpr(expr: BatchExpression): Boolean = {
   ...
   */
  def addExprTree(root: BatchExpression, ignoreLeaf: Boolean = true): Unit = {
    val skip = root.isInstanceOf[LeafBatchExpression] && ignoreLeaf
   ...
   */
  def getEquivalentExprs(e: BatchExpression): Seq[BatchExpression] = {
   ...
   */
  def getAllEquivalentExprs: Seq[Seq[BatchExpression]] = {
