...
abstract class SparkStrategies extends QueryPlanner[SparkPlan] {
  ...
  case class BasicOperators(conf:SQLConf) extends Strategy {
    // 开启向量化执行
    val vectorizeEnabled = conf.vectorizedExecutionEnabled
    // 开启向量化Sort
    val vectorizedSortEnabled = vectorizeEnabled &&
      conf.vectorizedSortEnabled &&
      !conf.vectorizedBatchSortEnabled
    // 开启向量化BatchSort
    val vectorizedBatchSortEnabled = vectorizeEnabled &&
      conf.vectorizedSortEnabled &&
      conf.vectorizedBatchSortEnabled &&
      !conf.vectorizedBatchSortUse0 &&
      !conf.vectorizedBatchSortUse2 &&
      !conf.vectorizedBatchSortUse3 &&
      !conf.vectorizedBatchSortUse4
    // 开启向量化BatchSort0
    val vectorizedBatchSortUse0 = vectorizeEnabled &&
      conf.vectorizedSortEnabled &&
      conf.vectorizedBatchSortEnabled &&
      conf.vectorizedBatchSortUse0
    // 是否开启向量化BatchSort2
    val vectorizedBatchSortUse2 = vectorizeEnabled &&
      conf.vectorizedSortEnabled &&
      conf.vectorizedBatchSortEnabled &&
      conf.vectorizedBatchSortUse2
    // 是否开启向量化BatchSort3
    val vectorizedBatchSortUse3 = vectorizeEnabled &&
      conf.vectorizedSortEnabled &&
      conf.vectorizedBatchSortEnabled &&
      conf.vectorizedBatchSortUse3
    // 是否开启向量化BatchSort4
    val vectorizedBatchSortUse4 = vectorizeEnabled &&
      conf.vectorizedSortEnabled &&
      conf.vectorizedBatchSortEnabled &&
      conf.vectorizedBatchSortUse4
    // 支持BatchExchange
    ...
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      ...
      case logical.Sort(sortExprs, global, child) =>
        if (vectorizedBatchSortUse4) {
          execution.vector.PureBatchSort4(sortExprs, global, planLater(child)) :: Nil
        }  else {
