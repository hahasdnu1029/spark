...
@Private
public class BatchShuffleWriter<K, V> extends ShuffleWriter<K, V> {

  private final Logger logger = LoggerFactory.getLogger(BatchShuffleWriter.class);
  ...
  @Nullable private BatchShuffleExternalSorter sorter;
  ...
  private void open() throws IOException {
    assert (sorter == null);
    sorter = new BatchShuffleExternalSorter(
        memoryManager,
        blockManager,
        taskContext,
  ...
  public Option<MapStatus> stop(boolean success) {
    try {
      TaskMetrics metrics = TaskContext.get().taskMetrics();
      if (metrics != null) {
        metrics.incPeakExecutionMemory(getPeakMemoryUsedBytes());
      }
